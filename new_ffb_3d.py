...
class NewFFBBasis3D(FBBasis3D):
    ...
    def to_complex(self, v):
        '''
        Convert the coefficients from real spherical harmonics to complex spherical harmonics.
        '''
        # TODO better docs
        ...
        if v.dtype == np.float32:
            ...
            return "nothing" # TODO handle it better

        c = np.zeros_like(v, dtype=dt)
        ...
        for ell in range(1, self.ell_max+1):
            ...
            for m in range(1, ell+1):
                ...
                # lower and upper limit of coefficients with m > 0
                ll_m = idx + (ell + m)*k_max_ell
                ul_m = idx + (ell + m + 1)*k_max_ell

                # lower and upper limit of coefficients with m < 0
                ll_mm = idx + (ell - m)*k_max_ell
                ul_mm = idx + (ell - m+1)*k_max_ell

                # Calculate complex coefficients with m > 0
                c[..., ll_m:ul_m] = (-1)**m * ( v[..., ll_m:ul_m] + 1.0j * v[..., ll_mm:ul_mm]  ) / np.sqrt(2)

                # Calculate complex coefficients with m < 0
                c[..., ll_mm:ul_mm] =  ( v[..., ll_m:ul_m] - 1.0j * v[..., ll_mm:ul_mm]  ) / np.sqrt(2)

            idx += k_max_ell*(2*ell+1)
        ...
        for ell in range(0, self.ell_max+1):
            ...
            # lower and upper limit of coefficients with m = 0
            ll = idx + ell * k_max_ell
            ul = idx + (ell+1) * k_max_ell
    ...
    def to_real(self, c):
        '''
        Convert the coefficients from complex spherical harmonics to real spherical harmonics.
        '''
        # TODO better docs
        ...
        for ell in range(1, self.ell_max+1):
            ...
            for m in range(1, ell+1):
                ...
                # Calculate complex coefficients with m > 0
                v[..., ll_m:ul_m] = np.sqrt(2) * (-1)**m * np.real(c[..., ll_m:ul_m])

                # Calculate complex coefficients with m < 0
                v[..., ll_mm:ul_mm] = np.sqrt(2) * (-1)**m * np.imag(c[..., ll_m:ul_m])
